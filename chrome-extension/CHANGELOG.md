...
### 🐛 修复
- **修复"发送到手机"按钮误触发文件上传弹窗的问题**
  - 问题原因：content.js 的全局 click 监听器会检查点击元素附近是否有文件输入框
  - 当"发送到手机"按钮附近有隐藏的文件上传输入框时，会误触发拦截
  - 解决方案：给按钮和模态框添加 `data-qr-upload-ignore` 属性
  - content.js 会忽略带此属性元素的点击事件

### 🔧 技术改进
- 发送按钮添加 `data-qr-upload-ignore="true"`
- 模态框及其所有子元素添加 `data-qr-upload-ignore="true"`
- 按钮点击事件添加 `e.stopImmediatePropagation()` 阻止其他监听器
- 确保"发送到手机"功能不会与文件上传拦截功能冲突
